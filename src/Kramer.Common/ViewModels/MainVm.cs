...
namespace Kramer.Common.ViewModels
{
    public class MainVm : BindableBase
    {
        ...
        public async Task Init()
        {
            ...
            var url =
                string.Format(
                    "http://api.sr.se/api/v2/episodes/index?programid=83&fromdate={0}&todate={1}&urltemplateid=3&audioquality=hi&pagination=false&format=xml",
                    fromDate.ToSwedishDate(), 
                    untilDate.ToSwedishDate());
            await GetFeedAsync(url);
        }
